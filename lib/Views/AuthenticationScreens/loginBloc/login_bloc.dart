...
import 'package:bangla_bazar/ModelClasses/send_otp_response.dart';
  ...
  Stream<LoginState> mapEventToState(
    ...
    if (event is LoginUser) {
        ...
        if (isInternetConnected == false) {
          ...
          try {
            ...
            if (response != null) {
              ...
              yield VerifyEmailState(sendOTPResponse: sendOTPResponse);
            } else {
              yield ErrorState(error: 'Timeout');
            }
          } catch (e) {
      ...
      if (isInternetConnected == false) {
        ...
        try {
          dynamic response = await Repository().sendEmailOTP(
            email: event.email,
          );
          print('Bloc Response: ${jsonDecode(response.toString())}');
          ...
          if (response != null) {
            ...
            yield SendEmailOTPState(sendOTPResponse: sendOTPResponse);
          } else {
            yield ErrorState(error: 'Timeout');
          }
        } catch (e) {
